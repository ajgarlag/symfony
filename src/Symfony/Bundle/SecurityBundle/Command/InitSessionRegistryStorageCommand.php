 ...
 */
class InitSessionRegistryStorageCommand extends ContainerAwareCommand
{
    /**
     * @see Command
     */
    ...
    {
        $this
            ->setName('init:session-registry-storage')
            ->setDescription('Executes the SQL needed to generate the database schema required by the session registry storage.')
            ->setHelp(<<<EOT
The <info>init:session-registry-storage</info> command executes the SQL needed to
generate the database schema required by the session registry storage Doctrine DBAL implementation:

<info>./app/console init:session-registry-storage</info>
EOT
        );
    }
