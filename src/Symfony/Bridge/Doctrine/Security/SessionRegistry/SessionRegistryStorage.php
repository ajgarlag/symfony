...
class SessionRegistryStorage implements SessionRegistryStorageInterface
{
     ...
     * @return SessionInformation|null a SessionInformation object.
     */
    ...
    public function setSessionInformation(SessionInformation $sessionInformation)
    {
        ...
        if (null !== $mergeSql) {
            $this->connection->executeQuery(
                ...
                array(
                    ...
                    'last_used' => $sessionInformation->getLastRequest(),
        ...
        $updateStmt = $this->connection->prepare(
            "UPDATE $this->table SET username=:username, last_used=:last_used, expired=:expired WHERE session_id = :session_id"
        );
        ...
        $updateStmt->bindValue('last_used', $sessionInformation->getLastRequest(), 'datetime');
    ...
    private function instantiateSessionInformationFromResultSet($data)
    {
        return new SessionInformation(
            $data['session_id'],
            $data['username'],
            null === $data['last_used'] ? null : new \DateTime($data['last_used']),
    ...
    private function getMergeSql()
    {
        switch ($this->connection->getDatabasePlatform()->getName()) {
            case 'mysql':
                return "INSERT INTO $this->table (session_id, username, last_used, expired) VALUES (:session_id, :username, :last_used, :expired) ".
                    "ON DUPLICATE KEY UPDATE username = VALUES(username), last_used = VALUES(last_used), expired = VALUES(expired)";
            case 'oracle':
                // DUAL is Oracle specific dummy table
                return "MERGE INTO $this->table USING DUAL ON (session_id= :session_id) ".
                    "WHEN NOT MATCHED THEN INSERT (session_id, username, last_used, expired) VALUES (:session_id, :username, :last_used, :expired) ".
                    "WHEN MATCHED THEN UPDATE SET username = :username, last_used = :last_used, expired = :expired";
            case 'mssql':
            ...
            case 'sqlite':
                return "INSERT OR REPLACE INTO $this->table (session_id, username, last_used, expired) VALUES (:session_id, :username, :last_used, :expired)";
        }
    }
    ...
    public function collectGarbage($maxLifetime)
    {
        ...
        ;
        $queryBuilder->setParameter('last_used', time() - $maxLifetime);
