...
class UniqueEntityValidatorTest extends ConstraintValidatorTestCase
{
    ...
    public function testValidateArrayUniqueness()
    {
        $constraint = new UniqueEntity([
            'message' => 'myMessage',
            'fields' => ['[name]' => 'name'],
    ...
    public function testValidateMappingOfFieldNames()
    {
        ...
        $this->buildViolation('myMessage')
            ->atPath('property.path.primaryName')
            ->setParameter('{{ value }}', '"Foo"')
            ->setInvalidValue('Foo')
            ->setCause([$entity])
            ->setCode(UniqueEntity::NOT_UNIQUE_ERROR)
    ...
    public function testValidateMappingOfFieldNamesToArray()
    {
        $constraint = new UniqueEntity([
            'message' => 'myMessage',
            'fields' => ['[primaryName]' => 'name', '[secondaryName]' => 'name2'],
        ...
        $entity = new DoubleNameEntity(1, 'Foo', 'Bar');
        $dto = ['primaryName' => 'Foo', 'secondaryName' => 'Bar'];
    ...
    {
        $this->expectException('Symfony\Component\Validator\Exception\ConstraintDefinitionException');
        $this->expectExceptionMessage('Cannot read property "primaryName" from class "Symfony\Bridge\Doctrine\Tests\Fixtures\HireAnEmployee".');
        $constraint = new UniqueEntity([
            'message' => 'myMessage',
    ...
    public function testInvalidateArrayFieldName()
    {
        $this->expectException('Symfony\Component\Validator\Exception\ConstraintDefinitionException');
        $this->expectExceptionMessage('Cannot read path "[primaryName]" from "{"name":"Foo"}".');
        $constraint = new UniqueEntity([
            'message' => 'myMessage',
            'fields' => ['[primaryName]' => 'name'],
            'em' => self::EM_NAME,
    ...
    {
        $this->expectException('Symfony\Component\Validator\Exception\ConstraintDefinitionException');
        $this->expectExceptionMessage('The field "name2" is not mapped by Doctrine entity "Symfony\Bridge\Doctrine\Tests\Fixtures\SingleStringIdEntity".');
        $constraint = new UniqueEntity([
            'message' => 'myMessage',
            'fields' => ['name2'],
            'em' => self::EM_NAME,
